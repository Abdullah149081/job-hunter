...
const AppliedJobs = () => {
  ...
  const [appliedJobs] = useState(jobsApplied);
  const [applies, setApplies] = useState(appliedJobs);

  const handlerRemote = () => {
    const remote = appliedJobs.filter((app) => app.remote_or_onsite === "Remote");
    setApplies(remote);
  };
  const handlerOnsite = () => {
    const onsite = appliedJobs.filter((app) => app.remote_or_onsite === "Onsite");
    setApplies(onsite);
  };

  return (
    <div className="px-4 py-16  md:px-24 lg:px-28 lg:py-20">
      <p className="text-center text-4xl font-bold text-gray-800">{appliedJobs.length > 0 ? <span>Applied Jobs</span> : <span>You're not applied any job</span>}</p>
